 ...
 */
function getLinkUrl(dir: Direction) {
  if (dir === "next") {
    const el = document.querySelector(SELECTORS.next) as HTMLAnchorElement;
    return el ? el.href : null;
  } else {
    const el = document.querySelector(SELECTORS.prev) as HTMLAnchorElement;
    return el ? el.href : null;
  }
}
 ...
 * @param {Direction} dir
 */
function handleNavigation(dir: Direction) {
  const url = links[dir];
  if (url) {
